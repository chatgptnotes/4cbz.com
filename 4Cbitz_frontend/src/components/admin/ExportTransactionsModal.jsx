...
const ExportTransactionsModal = ({ isOpen, onClose }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await paymentsAPI.exportTransactions(startDate, endDate);
      ...
      if (response.success && response.data.transactions) {
        ...
        const excelData = transactions.map((t, index) => ({
          'S.No': index + 1,
          'User Name': t.user?.name || 'N/A',
          'Email': t.user?.email || 'N/A',
          'Contact Number': t.user?.contact_number || 'N/A',
          'Document': t.document?.title || 'Lifetime Subscription',
        ...
        <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md transform transition-all">
          ...
          <div className="px-6 py-4 space-y-4">
            <p className="text-sm text-gray-600">
              Select a date range to export transactions as an Excel file.
            </p>
